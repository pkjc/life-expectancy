...
public class LifeExpectancyService {
	...
	// new
	// LifeExpectancyService().getMinLifeExpectancyForFemalesByDob("1952-03-11");
	// }
	...
	// http://api.population.io:80/1.0/life-expectancy/total/male/United
	// Kingdom/1952-03-11/;
		...
		for (String country : countriesResponse.getBody().getCountries()) {
			...
			if (lifeExpectancyResponse != null) {
				...
				if (lifeExpectancyResponse.hasBody()) {
					// Double lifeExp =
					// Double.parseDouble(lifeExpectancyResponse.getBody().getRemainingLifeExpectancy());
					if (lifeExpectancyResponse.getBody() != null
							&& lifeExpectancyResponse.getBody().getRemainingLifeExpectancy() < minLifeExpectancy) {
	...
	public ResponseEntity<LifeExpectancy> getMinLifeExpectancyForFemalesByToday(String country, String dob) {
		ResponseEntity<LifeExpectancy> lifeExpResponse = getLifeExpectancyForCountryAndDob(country, dob);
		return lifeExpResponse;
	}
	...
	private ResponseEntity<Country> getCountriesFromApi() {
		...
		ResponseEntity<Country> response = restTemplate.exchange(uriForCountries.toString(), HttpMethod.GET, null,
				Country.class);
		return response;
	}
	...
	private ResponseEntity<LifeExpectancy> getLifeExpectancyForCountryAndDob(String country, String dob) {
		...
		try {
			...
			uriForLifeExpectancy.append(BASE_URL).append(LIFE_EXPECTANCY_ENDPOINT).append("/total/")
					.append(GENDER_FEMALE).append("/").append(country).append("/").append(dob);

			response = restTemplate.exchange(uriForLifeExpectancy.toString(), HttpMethod.GET, null,
					LifeExpectancy.class);

			logger.info("uriForLifeExpectancy formed: " + uriForLifeExpectancy);
		} catch (RestClientException e) {
			logger.info("Error occurred: while fetching life expectancy: " + e.getMessage());
		}
		return response;
	}
}
